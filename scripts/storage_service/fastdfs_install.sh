...
fastdfs_config(){
	...
	if [[ ${deploy_mode} = '1' ]];then
		...
		sed -i "s#^tracker_server.*#tracker_server=${local_ip}:22122#" ${home_dir}/etc/client.conf
		...
		sed -i "s#^tracker_server.*#\#tracker_server=${local_ip}:22122#" ${home_dir}/etc/storage.conf
		if [[ ${fastdht_enable} = 'yes' ]];then
