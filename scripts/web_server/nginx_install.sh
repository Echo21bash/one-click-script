...
nginx_env_load(){
	...
	soft_name=nginx
	program_version=('1.14' '1.15' '1.16' '1.17' '1.18' '1.19' '1.20')
...
nginx_compile(){
	...
	if [[ ${add_module[*]} =~ '1' ]];then
		...
		sed -i 's#CORE_INCS=.*#CORE_INCS="$CORE_INCS /usr/include/fastdfs /usr/include/fastcommon/"#' ${tar_dir}/${add_module_value}-1.22/src/config
	fi
	if [[ ${add_module[*]} =~ '2' ]];then
		...
		[[ ${detail_version_number} > 1.13.99 && ${detail_version_number} < 1.16.1 ]] && patch -p1 < ${tar_dir}/${add_module_value}-master/check_1.14.0+.patch
		[[ ${detail_version_number} > 1.16.0 ]] && patch -p1 < ${tar_dir}/${add_module_value}-master/check_1.16.1+.patch
